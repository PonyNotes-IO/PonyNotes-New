...
class SaberCoreCanvas extends StatelessWidget {
  const SaberCoreCanvas({
    ...
    this.drawBackground = true, // ✅ 是否绘制背景（默认true，如果需要将图片层置于笔迹层下方，则设为false）
    this.drawStrokes = true, // ✅ 是否绘制笔迹
  });
  ...
  final bool drawBackground; // ✅ 是否绘制背景
  final bool drawStrokes; // ✅ 是否绘制笔迹
  ...
  Widget build(BuildContext context) {
    ...
    LogUtils.debug(
        '🎨 [SaberCoreCanvas] build: page.strokes=${page.strokes.length}, currentStroke=$currentStroke, selectResult=$selectResult, isSelecting=$isSelecting');
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        LogUtils.debug(
            '📐 [SaberCoreCanvas] layout: scale=$scale, offset=($offsetX,$offsetY), size=(${drawWidth}x${drawHeight})');
        ...
        return Stack(
          children: [
            ...
            // 只有当 drawBackground 为 true 时才绘制 PDF 背景
            if (drawBackground && page.backgroundImage != null)
              _PdfBackground(
                key: ValueKey(
                    'pdf_bg_${page.backgroundImage!.pdfFilePath}_${page.backgroundImage!.pdfPageIndex}'),
            ...
            // 只有当 drawStrokes 为 true 或者需要在背景层绘制选择框时才创建 CustomPaint
            // 但为了简单起见，我们总是创建 CustomPaint，在 Painter 内部控制绘制
            CustomPaint(
              painter: _SaberCoreCanvasPainter(
                ...
                drawBackground: drawBackground, // ✅ 传递背景绘制标记
                drawStrokes: drawStrokes, // ✅ 传递笔迹绘制标记
              ),
...
class _SaberCoreCanvasPainter extends CustomPainter {
  _SaberCoreCanvasPainter({
    ...
    this.editingTextBoxId, // ✅ 正在编辑的文本框ID（用于避免重影）
    Listenable? repaint,
    this.drawBackground = true, // ✅ 是否绘制背景
    this.drawStrokes = true, // ✅ 是否绘制笔迹
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // ✅ 只有当 drawBackground 为 true 时才绘制背景
    if (drawBackground) {
      _drawBackground(
          canvas, Size(drawWidth, drawHeight), offsetX, offsetY, scale);
    }
    ...
    if (highlighterStrokes.isNotEmpty) {
      for (final stroke in highlighterStrokes) {
        ...
        _drawStrokePath(canvas, stroke, scale, offsetX, offsetY, drawColor,
    ...
    if (coreInfo.laserStrokes.isNotEmpty) {
      debugPrint(
          '🔴 [SaberCanvas] Drawing ${coreInfo.laserStrokes.length} laser strokes');
    }
    ...
    final Stroke? strokeToDraw =
        currentStrokeListenable?.value ?? currentStroke;
    if (strokeToDraw != null) {
      if (strokeToDraw.toolId == ToolId.highlighter) {
      ...
      } else if (strokeToDraw is LineStroke ||
          strokeToDraw is ArrowLineStroke) {
        // ✅ 直线和箭头：使用专门的绘制方法
        ...
        debugPrint(
            '🎨 [SaberCanvas] Drawing current stroke as incomplete: toolId=${strokeToDraw.toolId}, type=${strokeToDraw.runtimeType}');
    ...
    if (selectResult != null) {
      ...
      // 由于我们只传递了当前页面，所以直接绘制
      _drawSelection(
          canvas, selectResult!, scale, offsetX, offsetY, isSelecting);
    }

    // ✅ 绘制PDF文本选择区域
    if (pdfTextSelectionRect != null) {
      _drawPdfTextSelection(
          canvas, pdfTextSelectionRect!, scale, offsetX, offsetY);
    }
  ...
  void _drawBackground(Canvas canvas, Size drawSize, double offsetX,
  ...
  /// 绘制激光笔笔迹（发光效果）
  void _drawLaserStroke(Canvas canvas, Stroke stroke, double scale,
      double offsetX, double offsetY) {
    debugPrint(
        '🔴 [SaberCanvas] Drawing laser stroke: points=${stroke.points.length}');
    ...
    if (smoothPath.getBounds().isEmpty) {
      debugPrint(
          '🔴 [SaberCanvas] Laser stroke smooth path is empty, skipping');
      return;
    }
    ...
    if (innerPolygon.isNotEmpty) {
      ...
      final pointVectors =
          stroke.points.map((p) => pf.PointVector(p.dx, p.dy)).toList();
  ...
  void _drawStroke(Canvas canvas, Stroke stroke, double scale, double offsetX,
      double offsetY) {
    if (stroke.points.isEmpty ||
    ...
    if (stroke is LineStroke || stroke is ArrowLineStroke) {
      ...
      debugPrint(
          '🖼️ [SaberCanvas] Drawing saved regular stroke: toolId=${stroke.toolId}, type=${stroke.runtimeType}');
    }
    ...
    if (customPaint != null && stroke.toolId == ToolId.pencil) {
      _drawStrokePathWithPaint(
          canvas, stroke, scale, offsetX, offsetY, customPaint);
    } else {
      _drawStrokePath(
          canvas, stroke, scale, offsetX, offsetY, strokeColor, strokeWidth);
    }
  }
  ...
  void _drawStrokePathWithPaint(Canvas canvas, Stroke stroke, double scale,
      double offsetX, double offsetY, Paint paint) {
    if (stroke.points.isEmpty) {
      return;
    }
    ...
    if (smoothPath.getBounds().isEmpty) {
      if (stroke.points.isNotEmpty) {
        final Offset p =
            _transform(stroke.points.first, scale, offsetX, offsetY);
        canvas.drawCircle(
            p,
  ...
  /// 绘制正在绘制的笔迹（未完成状态）
  void _drawStrokeIncomplete(Canvas canvas, Stroke stroke, double scale,
      double offsetX, double offsetY) {
    if (stroke.points.isEmpty ||
        stroke.toolId == ToolId.highlighter ||
        stroke.toolId == ToolId.laserPointer ||
    ...
    // ✅ 如果铅笔有自定义 Paint，使用它；否则使用默认绘制
    if (customPaint != null && stroke.toolId == ToolId.pencil) {
      _drawStrokePathIncompleteWithPaint(
          canvas, stroke, scale, offsetX, offsetY, customPaint);
    } else {
      _drawStrokePathIncomplete(
          canvas, stroke, scale, offsetX, offsetY, strokeColor, strokeWidth);
  ...
  void _drawStrokePathIncompleteWithPaint(Canvas canvas, Stroke stroke,
      double scale, double offsetX, double offsetY, Paint paint) {
    if (stroke.points.isEmpty) {
      return;
    }
  ...
  /// 绘制未完成的笔迹路径（实时绘制，不闭合）
  void _drawStrokePathIncomplete(Canvas canvas, Stroke stroke, double scale,
      double offsetX, double offsetY, Color color, double strokeWidth) {
    if (stroke.points.isEmpty) {
      return;
    }
  ...
  /// 绘制笔迹路径（通用方法）
  /// 使用 perfect_freehand 生成平滑路径
  void _drawStrokePath(Canvas canvas, Stroke stroke, double scale,
      double offsetX, double offsetY, Color color, double strokeWidth) {
    if (stroke.points.isEmpty) {
      return;
    }
  ...
  /// ✅ 绘制直线笔迹
  void _drawLineStroke(Canvas canvas, Stroke stroke, double scale,
      double offsetX, double offsetY) {
      ...
      if (length > 0) {
        ...
        while (currentLength < length) {
          if (lineStroke.dashStyle == DashStyle.dashDot) {
            ...
            final dashEndLength =
                math.min(currentLength + currentDashLength, length);
    ...
    if (stroke is ArrowLineStroke) {
      debugPrint(
          '🎯 [SaberCanvas] Drawing arrow: arrowStyle=${stroke.arrowStyle}');
      if (stroke.arrowStyle == ArrowStyle.doubleArrow) {
        ...
        // ✅ 单向箭头：只在末端绘制
        debugPrint(
            '🎯 [SaberCanvas] Drawing single arrow: style=${stroke.arrowStyle}');
        _drawSingleArrow(canvas, start, end, lineStroke.strokeWidth,
  ...
  /// ✅ 绘制单个箭头（在直线的一端）
  void _drawSingleArrow(Canvas canvas, Offset start, Offset end,
  ...
  /// ✅ 绘制矩形笔迹（支持填充和描边）
  void _drawRectangleStroke(Canvas canvas, RectangleStroke stroke, double scale,
      double offsetX, double offsetY) {
    final rect = stroke.rect;
    debugPrint(
        '🎨🎨🎨 [SaberCanvas] _drawRectangleStroke: rect=$rect, isEmpty=${rect.isEmpty}, color=${stroke.color}, strokeWidth=${stroke.strokeWidth}');
    if (rect.isEmpty) {
      debugPrint(
          '⚠️⚠️⚠️ [SaberCanvas] _drawRectangleStroke: rect is empty, skipping');
      return;
    }
    ...
    if (stroke.fillColor != null) {
      ...
      debugPrint(
          '🎨🎨🎨 [SaberCanvas] Drew rectangle fill: ${stroke.fillColor}');
    }
    ...
    if (stroke.dashStyle != DashStyle.solid) {
      ...
      debugPrint(
          '🎨🎨🎨 [SaberCanvas] Drew rectangle dashed stroke: dashStyle=${stroke.dashStyle}');
    } else {
      canvas.drawRect(rect, strokePaint);
      debugPrint(
          '🎨🎨🎨 [SaberCanvas] Drew rectangle stroke: color=${stroke.color}, width=${stroke.strokeWidth}');
    }
    canvas.restore();
  }

  /// ✅ 绘制圆形/椭圆笔迹（支持填充和描边）
  void _drawCircleStroke(Canvas canvas, CircleStroke stroke, double scale,
      double offsetX, double offsetY) {
    debugPrint(
        '🎨🎨🎨 [SaberCanvas] _drawCircleStroke: points=${stroke.points.length}, color=${stroke.color}, strokeWidth=${stroke.strokeWidth}');
    if (stroke.points.length < 2) {
      debugPrint(
          '⚠️⚠️⚠️ [SaberCanvas] _drawCircleStroke: not enough points, skipping');
      return;
    }
  ...
  void _drawTriangleStroke(Canvas canvas, TriangleStroke stroke, double scale,
      double offsetX, double offsetY) {
  ...
  /// ✅ 绘制菱形笔迹（支持填充和描边）
  void _drawDiamondStroke(Canvas canvas, DiamondStroke stroke, double scale,
      double offsetX, double offsetY) {
  ...
  /// ✅ 绘制选择框
  void _drawSelection(Canvas canvas, SelectResult selectResult, double scale,
      double offsetX, double offsetY, bool isSelecting) {
    canvas.save();
    canvas.translate(offsetX, offsetY);
    canvas.scale(scale);
    ...
    if (isSelecting) {
      ...
      if (boundingBox != null) {
        ...
        // 绘制虚线边框（使用Path绘制虚线效果）
        final path = Path()..addRect(boundingBox.inflate(4.0 / scale));
  ...
  /// ✅ 绘制PDF文本选择区域
  void _drawPdfTextSelection(Canvas canvas, Rect selectionRect, double scale,
      double offsetX, double offsetY) {
    canvas.save();
    canvas.translate(offsetX, offsetY);
    canvas.scale(scale);
  ...
  /// ✅ 绘制文本框
  void _drawTextBox(Canvas canvas, saber_text.TextBox textBox, double scale,
      double offsetX, double offsetY, String? editingTextBoxId) {
    ...
    if (plainTextContent.isNotEmpty || textBox.text.isNotEmpty) {
      ...
      final baseTextStyle = (textBox.textStyle ??
              const TextStyle(
                color: Colors.black,
              ))
  ...
  bool shouldRepaint(covariant _SaberCoreCanvasPainter oldDelegate) {
    ...
    // 激光笔笔迹正在淡出时，强制重绘
    else if (coreInfo.laserStrokes.isNotEmpty ||
        oldDelegate.coreInfo.laserStrokes.isNotEmpty) {
      needsRepaint = true;
    ...
    // 页面笔迹数量变化时重绘（参考Saber原版，简化比较逻辑）
      ...
      needsRepaint = true;
      repaintReason =
          'strokes length changed (${page.strokes.length} vs ${oldDelegate.page.strokes.length})';
    }
    // 页面文本框变化时重绘
    ...
    if (needsRepaint) {
      debugPrint(
          '🖌️ [SaberCoreCanvasPainter] shouldRepaint: true - $repaintReason');
    }
  ...
  /// 检查文本框列表是否相等
  bool _areTextBoxesEqual(
      List<saber_text.TextBox> a, List<saber_text.TextBox> b) {
  ...
  bool _areSelectResultsEqual(SelectResult? a, SelectResult? b) {
    ...
    return a.pageIndex == b.pageIndex && _areStrokesEqual(a.strokes, b.strokes);
  }

...
class _HandwritingSaberPocPageState extends State<HandwritingSaberPocPage> {
  static const int _maxPdfFileSizeBytes = 3 * 1024 * 1024 * 1024;
  static const int _maxPdfPageCount = 600;
  ...
  Widget _buildImageWidget(
    ...
    final bool isSelectTool =
        _currentToolNotifier.value.toolId == ToolId.select;

    // ✅ 判断当前图片是否应该被激活
    // 激活条件：图片被选中 且 在选择工具模式下
    final bool shouldActivate =
        _selectedImageIdNotifier.value == image.id && isSelectTool;
    ...
    return CanvasImage(
      ...
      onTap: () {
        ...
        if (isSelectTool) {
          ...
          debugPrint(
              '🦋[HandwritingSaber] _buildImageWidget onTap: 不是选择工具，isSelectTool=$isSelectTool');
        }
      },
  ...
  Future<void> _importPdfFromFilePath(String pdfFilePath) async {
    try {
      ...
      final pdfFileSize = await File(pdfFilePath).length();
      if (pdfFileSize > _maxPdfFileSizeBytes) {
        throw Exception('PDF 文件不能超过 3GB');
      }
      ...
      if (pdfDocument.pages.length > _maxPdfPageCount) {
        final pageCount = pdfDocument.pages.length;
        await pdfDocument.dispose();
        throw Exception('PDF 页数不能超过 $_maxPdfPageCount 页（当前 $pageCount 页）');
      }
  ...
  Widget build(BuildContext context) {
    try {
      ...
      debugPrint('❌ [HandwritingSaber] Stack trace: $stackTrace');
      return Material(
        child: Center(
          child: SingleChildScrollView(
            child: Column(

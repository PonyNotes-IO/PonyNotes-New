...
class _ContinueWithMagicLinkOrPasscodePageState
  ...
  bool _resendSliderVerified = false;
  int _resendSliderResetNonce = 0;
  ...
  Future<void> _resendCode() async {
    if (!_resendSliderVerified) {
      setState(() => _errorMessage = '请先完成滑动验证后再重新获取验证码');
      return;
    }
    ...
    try {
      ...
      if (widget.onEnterPasscode != null) {
        ...
        if (stateAfterSend.emailError != null) {
          setState(
            () => _errorMessage = stateAfterSend.emailError ?? '发送失败，请检查邮箱或手机号',
          );
          _resetResendSliderCaptcha();
          return;
        }
      ...
      showToastNotification(
        message: isEmail ? '验证码已重新发送到您的邮箱' : '验证码已重新发送到您的手机',
      );
    } catch (e) {
      setState(() => _errorMessage = '重新发送失败: $e');
      _resetResendSliderCaptcha();
                  ...
                  if (mounted && context.mounted) {
                    try {
                      ...
                      if (!passwordIsSet) {
                        ...
                        // 直接进入主界面
                        Log.info(
                            '[ContinueWithMagicLinkOrPasscodePage] 用户未设置密码，由 DeepLinkHandler 处理设置密码流程');
                        final rootContext = navigator.context;
                        if (rootContext.mounted) {
                          getIt<AuthRouter>()
                              .goHomeScreen(rootContext, userProfile);
                        } else {
                          Log.error(
                              '🟢 [ContinueWithMagicLinkOrPasscodePage] rootContext 未 mounted');
                        }
                      } else {
                        // 用户已设置密码，直接进入主界面
                        final rootContext = navigator.context;
                      ...
                      Log.error(
                          '🟢 [ContinueWithMagicLinkOrPasscodePage] 导航失败: $e',
                          stackTrace);
                    }
                  } else {
                    Log.error(
                        '🟢 [ContinueWithMagicLinkOrPasscodePage] context 未 mounted');
                  }
                });
              },
              (_) {},
    ...
    return Column(
      ...
      children: [
        ...
        if (_countdown == 0) ...[
          const VSpace(10),
          SliderCaptcha(
            resetKey: _resendSliderResetNonce,
            onVerified: () {
              setState(() {
                _resendSliderVerified = true;
                _errorMessage = '';
              });
            },
          ),
  ...
  void _resetResendSliderCaptcha() {
    if (!mounted) {
      return;
    }
    setState(() {
      _resendSliderVerified = false;
      _resendSliderResetNonce++;
    });
  }

...
const androidPrivacyPolicy = '''欢迎你使用“小马笔记”（以下简称“本产品”）的产品与/或服务！
...
- 使用目的：用户购买套餐支付使用
- 处理的信息类型：支付宝 SDK    支付宝是否安装、授权请求状态标识、授权结果、临时授权码及错误码
- 触发条件：默认不自动唤起，用户在下单页主动勾选支付后触发
- 会在支付完成后回调并唤起小马笔记
- 调用频率：非周期性调用，不在后台持续运行。仅当用户主动点击相应功能时，按次调用对应 SDK。每次操作仅查询一次目标应用的安装状态，并在完成本次授权或支付所必需的范围内调用 SDK；成功、失败、取消或超时后立即结束调用
- 关闭方式：用户可在第三方应用授权或支付页面取消操作，或返回本应用结束本次调用
...
- 使用目的：方便用户使用微信登录
- 处理的信息类型：微信 SDK    微信是否安装、授权请求状态标识、授权结果、临时授权码及错误码
- 触发条件：默认不自动唤起，用户主动选择微信登录后触发
- 会在微信授权完成后回调并唤起小马笔记
- 调用频率：非周期性调用，不在后台持续运行。仅当用户主动点击相应功能时，按次调用对应 SDK。每次操作仅查询一次目标应用的安装状态，并在完成本次授权或支付所必需的范围内调用 SDK；成功、失败、取消或超时后立即结束调用
- 关闭方式：用户可在第三方应用授权页面取消操作，或返回本应用结束本次调用
...
- 使用目的：方便用户使用抖音登录
- 处理的信息类型：抖音 SDK    抖音是否安装、授权请求状态标识、授权结果、临时授权码及错误码
- 触发条件：默认不自动唤起，用户主动选择抖音登录后触发
- 会在抖音授权完成后回调并唤起小马笔记
- 调用频率：非周期性调用，不在后台持续运行。仅当用户主动点击相应功能时，按次调用对应 SDK。每次操作仅查询一次目标应用的安装状态，并在完成本次授权或支付所必需的范围内调用 SDK；成功、失败、取消或超时后立即结束调用
- 关闭方式：用户可在第三方应用授权页面取消操作，或返回本应用结束本次调用
五、我们如何保护和保存你的个人信息
...
（5）如果我们不再为你提供产品或服务。
（三）注销你的帐号
1．如果你希望终止我们的服务时，你可以通过访问APP里“我的”申请注销你的账户。
...
本隐私政策发布日期：2026年9月18日
本隐私政策生效日期：2026年9月18日
''';

class AndroidPrivacyDialog extends StatelessWidget {
  const AndroidPrivacyDialog({super.key});

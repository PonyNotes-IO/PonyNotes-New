...
async fn get_document_event_test() {
  ...
  let document = test.open_document(view.id, None).await;
...
async fn apply_document_event_test() {
  ...
  let view = test.create_document().await;
  let doc_id = view.id.clone();
  let document = test.open_document(doc_id.clone(, None)).await;
  ...
  let document = test.open_document(doc_id, None).await;
  let document_data = document.data;
...
async fn undo_redo_event_test() {
  ...
  test.apply_actions(payload).await;
  let block_count_after_insert = test.open_document(doc_id.clone(, None)).await.data.blocks.len();

  // undo insert action
  let can_undo = test.can_undo_redo(doc_id.clone()).await.can_undo;
  assert!(can_undo);
  test.undo(doc_id.clone()).await;
  let block_count_after_undo = test.open_document(doc_id.clone(, None)).await.data.blocks.len();
  ...
  let block_count_after_redo = test.open_document(doc_id.clone(, None)).await.data.blocks.len();

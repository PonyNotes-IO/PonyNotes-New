...
pub extern "C" fn html_to_markdown_parse(
  ...
  let html_content = unsafe { std::slice::from_raw_parts(html_data, html_size) };

  let filename_str = unsafe { CStr::from_ptr(filename).to_string_lossy().to_string() };
...
pub extern "C" fn html_to_markdown_parse_string(
  ...
  let html_str = unsafe { CStr::from_ptr(html_content).to_string_lossy().to_string() };

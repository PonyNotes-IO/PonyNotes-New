...
fn main() -> Result<()> {
  ...
  // 读取HTML文件
  let html_content = fs::read_to_string(input_path).context("无法读取HTML文件")?;
  ...
  let markdown_content = match mode {
    "smart" => convert_html_to_markdown(&html_content, filename).context("智能HTML解析失败")?,
    "source" => create_html_viewer_content(filename, &html_content),
    "legacy" => {
      // 简单的传统解析：移除HTML标签

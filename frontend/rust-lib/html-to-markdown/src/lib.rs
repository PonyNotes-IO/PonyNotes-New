...
impl HtmlToMarkdownConverter {
  ...
  fn extract_title(&self, html_content: &str, filename: &str) -> String {
    // 尝试从title标签获取
    if let Some(captures) = Regex::new(r"<title[^>]*>(.*?)</title>")
      .unwrap()
    ...
    // 尝试从第一个h1标签获取
    if let Some(captures) = Regex::new(r"<h1[^>]*>(.*?)</h1>")
      .unwrap()
      .captures(html_content)
    {
  ...
  fn process_html_content(&self, html_content: &str) -> Result<String> {
    ...
    // 移除script和style标签
    content = Regex::new(r"<script[^>]*>.*?</script>")
      .unwrap()
      .replace_all(&content, "")
      .to_string();
    content = Regex::new(r"<style[^>]*>.*?</style>")
      .unwrap()
      .replace_all(&content, "")
      .to_string();
...
pub fn create_html_viewer_content(filename: &str, html_content: &str) -> String {
  ...
  // 限制显示的HTML长度
  let preview_length = if html_content.len() > 5000 {
    5000
  } else {
    html_content.len()
  };
  ...
  result.push_str("\n```\n\n");
  result.push_str(
    "*提示：此文档显示的是HTML源代码。要查看渲染后的效果，请在浏览器中打开原始HTML文件。*",
  );

  ...
  pub async fn handle(&self, event: TemplateEventPB, payload: Vec<u8>) -> FlowyResult<Vec<u8>> {
    match event {
      ...
      TemplateEventPB::AddToMyTemplates => {
        let payload =
          AddTemplateToMyTemplatesPayloadPB::try_from(payload.as_slice()).map_err(|e| {
            flowy_error::FlowyError::from(anyhow::anyhow!("Failed to parse payload: {}", e))
          })?;
      ...
      TemplateEventPB::RemoveFromMyTemplates => {
        ...
        self
          .manager
          .remove_from_my_templates(&payload.template_id)
          .await?;
        Ok(vec![])
      },
      ...
      TemplateEventPB::GetTemplatesByCategory => {
        let payload = TemplateCategoryPB::try_from(payload.as_slice()).map_err(|e| {
          flowy_error::FlowyError::from(anyhow::anyhow!("Failed to parse payload: {}", e))
        })?;
        let templates = self
          .manager
          .get_templates_by_category(&payload.category)
          .await?;
        let repeated = RepeatedTemplateItemPB { items: templates };
        Ok(repeated.try_into()?)
      },
      TemplateEventPB::SearchTemplates => {
        let payload = TemplateSearchPB::try_from(payload.as_slice()).map_err(|e| {
          flowy_error::FlowyError::from(anyhow::anyhow!("Failed to parse payload: {}", e))
        })?;

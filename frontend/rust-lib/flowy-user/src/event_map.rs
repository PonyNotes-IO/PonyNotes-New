...
pub fn init(user_manager: Weak<UserManager>) -> AFPlugin {
  ...
  AFPlugin::new()
    ...
    .event(UserEvent::GetTeams, get_teams_handler)
    .event(UserEvent::GetTeamACL, get_team_acl_handler)
    .event(UserEvent::UpdateTeamACL, update_team_acl_handler)
      // Workspace
    ...
    .event(UserEvent::VerifyAndBindPhone, verify_and_bind_phone_handler)
}
...
pub enum UserEvent {
  ...
  #[event(input = "UserWorkspaceIdPB", output = "RepeatedTeamPB")]
  GetTeams = 120,

  #[event(input = "TeamIdPB", output = "TeamACLPB")]
  GetTeamACL = 121,
  ...
  #[event(input = "VerifyAndBindPhonePB")]
  VerifyAndBindPhone = 66,
}

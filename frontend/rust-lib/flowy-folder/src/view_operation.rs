...
pub(crate) fn create_view(uid: i64, params: CreateViewParams, layout: ViewLayout) -> View {
  ...
  let extra = match params.layout {
    ...
    ViewLayoutPB::Notebook => {
      ...
      extra_map.insert(
        "view_type".to_string(),
        serde_json::Value::String("notebook".to_string()),
      );
      Some(serde_json::to_string(&extra_map).unwrap_or_default())
    },
    ViewLayoutPB::Whiteboard => {
      ...
      extra_map.insert(
        "view_type".to_string(),
        serde_json::Value::String("whiteboard".to_string()),
      );
      Some(serde_json::to_string(&extra_map).unwrap_or_default())
    },
    ViewLayoutPB::Document => {
      // Check if this is a handwriting_native view (from meta or extra)
      let view_type = params.meta.get("view_type").map(|s| s.clone()).or_else(|| {
      ...
      if let Some(view_type) = view_type {
        if view_type == "handwriting_native" {
          ...
          extra_map.insert(
            "view_type".to_string(),
            serde_json::Value::String("handwriting_native".to_string()),
          );
          Some(serde_json::to_string(&extra_map).unwrap_or_default())
        } else {
          params.extra
